...
trait SimplicatorHive {
  ...
  private def getMatchParameters[T](cmd: HiveCommand[T]): Match = {
    cmd match {
      ...
      case c: AddCustomField => Match(s"/contacts/${c.contactId}/custom.*${urlEncode(c.modifiedAt.toString)}", Seq(c.customField.field, c.customField.value), RequestMethod.POST)
      ...
      case c: UpdateCustomField => Match(s"/contacts/${c.contactId}/custom/${c.customFieldId}.*${urlEncode(c.modifiedAt.toString)}", Seq(c.customField.field, c.customField.value), RequestMethod.PUT)
